...
const AddNewBackModel = ({ initialData }) => {
  const {
    ...
    canvasRef,
    preview,
  } = AddNewBackModelLogic(initialData);

  return (
    <div>
      <h1 className="h2 border-bottom pb-2">
        {initialData != undefined ? "Edit Back" : "Add New Back"} Model
      </h1>
              ...
              {[...Array(3)].map((_, index) => (
                <React.Fragment key={index}>
                  <Form.Group>
                    <Form.Control
                      type="text"
                      id={`WatermarkText${index + 1}`}
                      placeholder={`Enter Watermark Text ${index + 1}`}
                    />
                  </Form.Group>
                  <Form.Group>
